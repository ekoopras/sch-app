        ...
        <div class="space-y-10 md:space-y-28">
            ...
            <div class="flex flex-col md:grid md:grid-cols-12 gap-5 md:gap-16 items-stretch md:items-center border-b border-gray-100 pb-10 md:border-none md:pb-0"
                ...
                <div class="flex-1 md:col-span-8 flex flex-col justify-center px-1 md:px-0">
                    ...
                    <p class="text-gray-600 text-xs sm:text-sm md:text-base mt-2 md:mt-4 leading-relaxed line-clamp-2 md:line-clamp-5">
                        @php
                        // 1. Hapus tag <img> secara total beserta atributnya (alt/title bawaan Filament yang berisi nama file)
                        $cleanContent = preg_replace('/<img[^>]*>/i', '', $item->content);

                            // 2. Bersihkan nama file mentah yang tersisa (hanya jika berdiri sendiri/dipisah spasi)
                            $cleanContent = preg_replace('/\b[\w\-\.\+]+\.(jpg|jpeg|png|gif|webp|svg)\b/i', '', $cleanContent);

                            // 3. Potong tag HTML lain dan batasi karakter
                            $finalExcerpt = Str::limit(strip_tags($cleanContent), 280);
                            @endphp
